...
pub fn arguments(metadata: &constant::MetaData) -> settings::Config {
    ...
    while i < args.len() {
        match args[i].as_str() {
            "-h" | "--help" => {
                ...
                --filename/-f: Object name to upload in s3 (eg: list.html)\tFallback: list.html\n\
    ...
    settings::parse_config(bucket, region, prefix, ignore, filename)
}
